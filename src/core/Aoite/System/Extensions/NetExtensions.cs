...
namespace System
{
    ...
    public static class NetExtensions
    {
        ...
        public static void Connect(this Socket server, EndPoint endPoint, int millisecondsTimeout)
        {
            var asyncResult = server.BeginConnect(endPoint, null, null);
            if(!asyncResult.AsyncWaitHandle.WaitOne(millisecondsTimeout)) throw new SocketException((int)SocketError.SocketError);
        ...
        /// <param name="millisecondsTimeout">等待的毫秒数，或为 <see cref="System.Threading.Timeout.Infinite"/> (-1)，表示无限期等待。</param>
        public static void Connect(this TcpClient server, string host, int port, int millisecondsTimeout)
        {
            var asyncResult = server.BeginConnect(host, port, null, null);
            if(!asyncResult.AsyncWaitHandle.WaitOne(millisecondsTimeout)) throw new SocketException((int)SocketError.TimedOut);
            server.EndConnect(asyncResult);
        }
        ...
        /// <param name="millisecondsTimeout">等待的毫秒数，或为 <see cref="System.Threading.Timeout.Infinite"/> (-1)，表示无限期等待。</param>
        public static void Connect(this TcpClient server, IPEndPoint endPoint, int millisecondsTimeout)
        {
