...
namespace System
{
    ...
    public static class CommonExtensions
    {
        ...
        public static TTarget CopyTo<TTarget>(this object source, TTarget target, CopyToStrategy targetStrategy = CopyToStrategy.Default)
        {
            ...
            var sMapper = Aoite.Data.EntityMapper.Create(source.GetType());
            var tMapper = Aoite.Data.EntityMapper.Create(target.GetType());
            foreach(var sProperty in sMapper.Properties)
            {
                var tProperty = tMapper[sProperty.Name];
                if(tProperty == null) continue;
                if(targetStrategy == CopyToStrategy.OnlyPrimaryKey && !tProperty.Column.IsPrimaryKey
                    || (targetStrategy == CopyToStrategy.ExcludePrimaryKey && tProperty.Column.IsPrimaryKey)
