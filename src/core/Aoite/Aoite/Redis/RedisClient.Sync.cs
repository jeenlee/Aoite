using Aoite.Redis.Internal.Commands;
using System;
...
namespace Aoite.Redis
{
    public partial class RedisClient
    {
        ...
        static Result CreateResult(string status, string ok = OKStatus)
        {
            if(status == ok) return Result.Successfully;
        ...
        public Result Auth(string password)
        {
            return CreateResult(Write(RedisCommand.Auth(password)));
        }

        /// <summary>
        /// 回显输入的字符串。
        /// </summary>
        /// <param name="message">回显的字符串。</param>
        /// <returns>返回回显的字符串。</returns>
        ...
        public Result Ping()
        {
            return CreateResult(Write(RedisCommand.Ping()), "PONG");
        }
        ...
        public Result Quit()
        {
            var r = CreateResult(Write(RedisCommand.Quit()));
            _connection.Dispose();
            return r;
        }
        ...
        /// 选择并切换数据库。
        /// </summary>
        /// <param name="index">大于 0 的数据库索引。</param>
        /// <returns>返回一个结果。</returns>
        public Result Select(int index)
        {
            return CreateResult(Write(RedisCommand.Select(index)));
        }
        ...
        /// 删除一个或多个 Redis 键。
        /// </summary>
        /// <param name="keys">删除 Redis 键的数组。</param>
        ...
        /// <summary>
        /// 序列化给定 <paramref name="key"/> ，并返回被序列化的值，使用 RESTORE 命令可以将这个值反序列化为 Redis 键。
        /// </summary>
        /// <param name="key">Key to dump</param>
        /// <returns>如果 <paramref name="key"/> 不存在，那么返回 null 。否则，返回序列化之后的值。</returns>
        ...
        {
            return Write(RedisCommand.HGetBytes(key, field));
        }
        ...
        {
            return Write(RedisCommand.HGetAllBytes(key));
        }
        ...
        {
            return Write(RedisCommand.GetBytes(key));
        }
        /// <summary>
