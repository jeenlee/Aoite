...
namespace Aoite.Data
{
    ...
    public class MsSqlTestManager : TestManagerBase
    {
        ...
        private string _DbOwnerUser;
        /// <summary>
        /// 获取可读写的用户名称。
        /// </summary>
        public string DbOwnerUser { get { return this._DbOwnerUser; } }

        private string _ReadonlyUser;
        /// <summary>
        /// 获取只读的用户名称。
        /// </summary>
        public string ReadonlyUser { get { return this._ReadonlyUser; } }
        ...
        public MsSqlTestManager(string connectionString, string server)
        {
            ...
            this._DbOwnerUser = Guid.NewGuid().ToString();
            this._ReadonlyUser = Guid.NewGuid().ToString();
 ...
 LOG ON ( NAME = N'{0}_log', FILENAME = N'{1}\{0}_log.ldf' , SIZE = 2048KB , FILEGROWTH = 10%)";
            using(var context = m_masterEngine.Context)
            {
                context.Open();
                context
                    .Execute(sql.Fmt(this._DatabaseName, dbDire))
                    .ToNonQuery()
                    .ThrowIfFailded();
                CreateUser(this._DatabaseName, this._DbOwnerUser, false, context);
                CreateUser(this._DatabaseName, this._ReadonlyUser, true, context);
            }

            this.Manager = new DbEngineManager();
            this.Manager.Add(Db.NameWithDefualtEngine, this.Engine = new MsSqlEngine(server, this._DatabaseName, this._DbOwnerUser, "123456"));
            this.Manager.Add(Db.NameWithReadonlyEngine, new MsSqlEngine(server, this._DatabaseName, this._ReadonlyUser, "123456"));
        }
        ...
        protected override void DisposeManaged()
        {
            ...
            using(var context = this.m_masterEngine.Context)
            {
                context.Open();
                context.ContextConnection.ChangeDatabase("master");
                context.Execute("ALTER DATABASE [{0}] SET SINGLE_USER WITH ROLLBACK IMMEDIATE".Fmt(this._DatabaseName)).ToNonQuery().ThrowIfFailded();
                context.Execute("DROP DATABASE [{0}]".Fmt(this._DatabaseName)).ToNonQuery().ThrowIfFailded();
                context.Execute("DROP LOGIN [{0}]".Fmt(this._DbOwnerUser)).ToNonQuery().ThrowIfFailded();
                context.Execute("DROP LOGIN [{0}]".Fmt(this._ReadonlyUser)).ToNonQuery().ThrowIfFailded();
            }
