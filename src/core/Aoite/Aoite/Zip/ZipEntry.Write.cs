...
namespace Aoite.Zip
{
    public partial class ZipEntry
    {
        ...
        private bool WantReadAgain()
        {
            ...
            if (CompressionLevel == Aoite.Zlib.CompressionLevel.None) return false;
        ...
        private void MaybeUnsetCompressionMethodForWriting(int cycle)
        {
            ...
            if (CompressionLevel == (short)Aoite.Zlib.CompressionLevel.None &&
                CompressionMethod == Aoite.Zip.CompressionMethod.Deflate)
                _CompressionMethod = 0x00;
        ...
        internal void WriteHeader(Stream s, int cycle)
        {
            ...
            // LastMod
            _TimeBlob = Aoite.Zip.SharedUtilities.DateTimeToPacked(LastModified);
        ...
        Int32 FigureCrc32()
        {
            if (_crcCalculated == false)
            {
                ...
                {
                    var output = new Aoite.Crc.CrcCalculatorStream(Stream.Null);
        ...
        private void _WriteEntryData(Stream s)
        {
            ...
            try
            {
                ...
                var output = new Aoite.Crc.CrcCalculatorStream(compressor, true);
        ...
        internal void FinishOutputStream(Stream s,
                                         ...
                                         Stream encryptor,
                                         Stream compressor,
                                         Aoite.Crc.CrcCalculatorStream output)
        {
            ...
            if ((compressor as Aoite.Zlib.DeflateStream) != null)
                compressor.Close();
#if BZIP
...
#if !NETCF
            else if ((compressor as Aoite.Zlib.ParallelDeflateOutputStream) != null)
                compressor.Close();
#endif
            ...
            if (_UncompressedSize == 0 && _CompressedSize == 0)
            {
                ...
                if (_Password != null)
                {
                    ...
                    if (Encryption != EncryptionAlgorithm.None)
                    {
                        ...
                        // workitem 10178
                        Aoite.Zip.SharedUtilities.Workaround_Ladybug318918(s);
        ...
        internal void PrepOutputStream(Stream s,
                                       ...
                                       out Aoite.Crc.CrcCalculatorStream output)
        {
            ...
            // Wrap a CrcCalculatorStream around that.
            // This will happen BEFORE compression (if any) as we write data out.
            output = new Aoite.Crc.CrcCalculatorStream(compressor, true);
        }
        ...
        {
            if (_CompressionMethod == 0x08 && CompressionLevel != Aoite.Zlib.CompressionLevel.None)
            {
#if !NETCF
                     ...
                     _container.ParallelDeflateThreshold > 0L))
                {
                    ...
                    if (_container.ParallelDeflater == null)
                    {
                        _container.ParallelDeflater =
                            new Aoite.Zlib.ParallelDeflateOutputStream(s,
                                                                       CompressionLevel,
                                                                       _container.Strategy,
                                                                       true);
                    ...
                    Aoite.Zlib.ParallelDeflateOutputStream o1 = _container.ParallelDeflater;
                    o1.Reset(s);
                    return o1;
                }
#endif
                var o = new Aoite.Zlib.DeflateStream(s, Aoite.Zlib.CompressionMode.Compress,
                                                     CompressionLevel,
                                                     true);
                if (_container.CodecBufferSize > 0)
