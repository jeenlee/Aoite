...
namespace fastJSON
{
    internal sealed class JSONSerializer
    {
        ...
        private void WriteValue(object obj)
        {
            ...
            {
                if((char)obj == '\0') _output.Append("null");
                else WriteStringFast(obj.ToString());
            }
            ...
            {
                var v = (long)obj;
                if(v > JSON.MaxNumber || v < JSON.MinNumber) WriteStringFast(v.ToString(NumberFormatInfo.InvariantInfo));
                else _output.Append(v);
            }
            else if(obj is ulong)
            {
                var v = (ulong)obj;
                if(v > JSON.MaxNumber) WriteStringFast(v.ToString(NumberFormatInfo.InvariantInfo));
                else _output.Append(v);
            }
      
            else if(
                obj is int || obj is double ||
        ...
        private void WriteEnum(Enum e)
        {
            ...
            if(_params.UseValuesOfEnums)
            {
                ...
                if(eType == Types.UInt64 || eType == Types.Int64)
                {
                    this.WriteStringFast(e.ToString("D"));
                }
                else _output.Append(e.ToString("D"));
            }
            else
                WriteStringFast(e.ToString().ToCamelCase());
        }
        ...
        private void WriteObject(object obj)
        {
            ...
            for(int ii = 0; ii < c; ii++)
            {
                ...
                else
                {
                    ...
                    WritePair(p.Name.ToCamelCase(), o);
